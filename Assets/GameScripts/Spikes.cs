...
public class Spikes : MonoBehaviour {
	...
	void Start () {
        ...
        pr_Tf_SonSpikes = pr_Tf_Spikes.FindChild("moving_spikes_b").GetComponent<Transform>();
        pr_V3_normalPos = pr_Tf_SonSpikes.position;
        pr_V3_TargetPos = pr_Tf_SonSpikes.position + new Vector3(0, 0.15f, 0);
    ...
    private IEnumerator Up()
    {
        while(true)
        {
            ...
            if(pr_Tf_SonSpikes.position.y-pr_Tf_Spikes.position.y>0.15)
            {
                break;
            }
            yield return null;
